...
const MapTest = (props) => {
  ...
  const handleButton = async (e) => {
    try {
      const res = await axios.get(nodeURL + "/naver/", {
        params: {
          query: e,
        },
      });
  ...
  const Map = () => {
    ...
    function displayMarker(locPosition, message) {
      ...
      var infowindow = new kakao.maps.InfoWindow({
        content: '<span class="info-title">말풍선타이틀</span>',
    ...
    function displayMarker(place, normalOrigin, clickOrigin) {
      ...
      var normalImage = new kakao.maps.MarkerImage(imageSrc2, imageSize),
        clickImage = new kakao.maps.MarkerImage(imageSrc, imageSize);
      ...
      function displayPlaceInfo(place) {
        var content = '<div class="marker_text">' + place.place_name + "</div>";
